...
const { getDbConnection, userReg, userLoginAct, savePrediction, savePrediction2 } = require('./src/components/db');
...
app.post('/api/register', async (req, res) => {
    ...
    const registrationResult = await userReg(username, email, password);
...
app.post('/api/login', async (req, res) => {
    ...
    const loginResult = await userLoginAct(username, password);
    if (loginResult > 0) {
...
// Health Data Submission (requires user authentication in a real app)
app.post('/api/health', async (req, res) => {
    try {
        const userId = req.body.userId; // Assuming you send userId for simplicity
        const { welcome,muscleOrWeightGoal,motivationLevel,willingToDo,profile,dietaryRestrictions,dietaryOptions,medicalConditions} = req.body;
        ...
        res.status(201).json({ message: 'Health profile submitted successfully' });
    } catch (error) {
        console.error('Error submitting health profile:', error);
        res.status(500).json({ message: 'Failed to submit health profile' });
    }
});
